...
VarDecl : IdentList COLONSY Type SCOLONSY {for(unsigned int i = 0; i < $1->size(); i++){ symbol_table.addExpr($1->at(i), new Express($3, symbol_table.offset, true));} }
        ;
...
LValue : LValue DOTSY IDENTSY {}
       | LValue LBRACKETSY Expression RBRACKETSY {if(!$3->regist){ $$ = new std::string(*$1 + '[' + std::to_string($3->raw_val) + ']'); } else { std::string* temp = new std::string(name_ref()); symbol_table.addExpr(*temp, arLvalHelper($3, *$1)); $$ = temp; }}
